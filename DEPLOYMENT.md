...
| Node version | **22.19.0** | Pinned via `.node-version` (Cloudflare Pages reads this automatically) and mirrored in `package.json`'s `engines.node`. Required because `astro@7`, `@astrojs/compiler-rs`, `@astrojs/prism`, `cookie@2`, and `undici@8` all declare a minimum Node engine above 20 — a real Cloudflare build log showed Node 20.20.2 being used and failing engine checks against this dependency tree. **If a Cloudflare build ever shows a Node version other than 22.19.0 again**, `.node-version` is being overridden — set a `NODE_VERSION=22.19.0` environment variable in the Pages dashboard (Settings → Environment variables) to force it. |
